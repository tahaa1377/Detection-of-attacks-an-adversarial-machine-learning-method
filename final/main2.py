...
def extract_features(packet):
    ...
    my_model_attack = Model_Attack_Cat()
    ...
    for batch_x in test_attack_cat_dataloader:
        out = my_model_attack(batch_x)
        print(out)
        max_elements, max_index = torch.max(out, 1)
        # print(max_elements)
        # print(max_index)
        define_attck_category(max_index)
        print("------------------------------")

def define_attck_category(index):
    if index == 0:
        print("Analysis")
    elif index == 1:
        print("Backdoor")
